...
// ROM code search algorithm, tree search method
uint8_t DS18B20::oneWireSearch(uint8_t romCommand) {
    ...
    for (uint8_t bitPosition = 0; bitPosition < 64; bitPosition++) {
		...
		// Take the source code and the inverse code line and: 1,1 ----> No drive reply
		...
		// Take the source code and the inverse code line and: 0,0 ----> There are drive reply, device send 0 or send 1 (sized-multiple)
        if (!currentBit && !currentBitComp) {  
		...
		// Take the source code and the inverse code line and: 0,1 or 1,0 ----> There are drive reply
		else {
	...
	// Determine whether all tests are complete.
    if (!lastDiscrepancy) {
        lastDevice = 1;
    }
